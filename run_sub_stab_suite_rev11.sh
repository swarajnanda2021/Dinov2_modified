#!/bin/bash
# run_sub_stab_suite_rev11.sh -- parametrized launcher for the "new-baseline" ViT-B ablations.
#
...
exp_name="FMC_ViT-B_stab_${RUN}_rev11"
exp_dir="$BASE_DIR/$exp_name"

echo "========================================"
echo "Setup: $exp_name  (branch: $BRANCH)"
echo "  ViT-B/16 fixed-loader recipe (== rev3/rev5/rev6/rev7) + ONE ingredient: $RUN"
echo "  (rev11 = rev10 + thinned tilt reads typicality_a; bc_thinned_hi is a REAL a=1.0 arm, oversample 6.0)"
echo "========================================"
...
echo "  Verifying rev11 hand-rolled DP + loss tuning + analytic w_max + thinned tilt fix is present..."
...
echo "    rev11 hand-rolled DP + loss tuning + analytic w_max + thinned tilt fix verified."
...
case "$RUN" in
  ...
  bc_thinned_lo)
    ...
    ensure_arg typicality_c_frac            0.25
    ensure_arg typicality_radius_mult       1.5
    ensure_arg thin_oversample_factor       3.0                 # REV10: analytic w_max -> accept ~0.49 -> pool 3x N (768) gives ~376 admits vs N=256 (~8sigma). Was 6.0 (needed only because frozen w_max starved accept to ~0.15).
  ...
  bc_thinned_hi)
    ...
    ensure_arg typicality_c_frac            0.25
    ensure_arg typicality_radius_mult       1.5
    ensure_arg thin_oversample_factor       6.0                 # REV11: tilt fix live -> hi admits at a=1.0. a=1.0 ~halves acceptance (accept ~0.24-0.35, chi ~3-4), so pool must be 6x N (was 3.0 only because the old a=0.5 hardcode kept accept ~0.49). Watch thin_accept: if it dips so admits < N=256, RAISE further.
