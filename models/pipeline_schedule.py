...
class PipelineSchedule:
    ...
    def forward_pass(
        ...
        with torch.no_grad():
            if self.is_first_stage:
                ...
                teacher_input, teacher_attn_bias = self._recv_from_prev_stage()
                teacher_output, teacher_attn_bias_out = self.teacher_stage(
                    teacher_input, 
                    token_masks=None, 
                    attn_bias=teacher_attn_bias,
                    use_packing=self.use_packing
        ...
        if self.is_first_stage:
            ...
            student_input, student_attn_bias = self._recv_from_prev_stage()
            student_output, student_attn_bias_out = self.student_stage(
                student_input, 
                token_masks=None, 
                attn_bias=student_attn_bias,
                use_packing=self.use_packing
        ...
        with torch.no_grad():
            if self.is_first_stage:
                ...
                teacher_ibot_features = self.teacher_stage(
                    teacher_ibot_input, 
                    token_masks=None, 
                    attn_bias=teacher_ibot_attn_bias,
                    use_packing=self.use_packing
                )
                # CRITICAL: Clear input immediately
                ...
                teacher_ibot_input, teacher_ibot_attn_bias = self._recv_from_prev_stage()
                teacher_ibot_output, teacher_ibot_attn_bias_out = self.teacher_stage(
                    teacher_ibot_input, 
                    token_masks=None, 
                    attn_bias=teacher_ibot_attn_bias,
                    use_packing=self.use_packing
        ...
        if self.is_first_stage:
            ...
            student_ibot_features = self.student_stage(
                student_ibot_input, 
                token_masks=None, 
                attn_bias=student_ibot_attn_bias,
                use_packing=self.use_packing
            )
            # CRITICAL: Clear input immediately
            ...
            student_ibot_input, student_ibot_attn_bias = self._recv_from_prev_stage()
            student_ibot_output, student_ibot_attn_bias_out = self.student_stage(
                student_ibot_input, 
                token_masks=None, 
                attn_bias=student_ibot_attn_bias,
                use_packing=self.use_packing
        ...
        if self.is_last_stage:
            print(f"[Rank {self.local_rank}] Computing losses...", force=True, flush=True)
            ...
            print(f"[Rank {self.local_rank}] Losses computed: {list(losses.keys())}", force=True, flush=True)
            return losses
        else:
            return None
    ...
    def _forward_through_pipeline(
        ...
        if self.is_first_stage:
            ...
            if self.debug:
                print(f"[Rank {self.local_rank}] First stage output: {output.shape}", force=True, flush=True)
            ...
            if self.debug:
                print(f"[Rank {self.local_rank}] Middle stage output: {output.shape}", force=True, flush=True)
            ...
            if self.debug:
                print(f"[Rank {self.local_rank}] Last stage output: {type(output)}", force=True, flush=True)
    ...
    def _compute_losses(
        ...
        if self.debug:
            print(f"[Rank {self.local_rank}] Computed losses: {list(losses.keys())}", force=True, flush=True)
    ...
    def _sync_gradients_data_parallel(self, model):
        ...
        if self.debug:
            print(f"[Rank {self.local_rank}] Synced gradients across data parallel group", force=True, flush=True)
    ...
    def update_teacher_ema(self, momentum: float):
        ...
        if self.debug:
            print(f"[Rank {self.local_rank}] Teacher EMA update completed", force=True, flush=True)