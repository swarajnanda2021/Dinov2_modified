#!/bin/bash
# run_sub_stab_suite_rev12.sh -- parametrized launcher for the "new-baseline" ViT-B ablations.
#
# REV12 = REV11 + BANK M-SCALING for the thinned arms (finer typicality resolution):
#   - bc_thinned_lo/hi go to M = 32768 (4x). M and a are the only free knobs; every derived
#     constant is co-scaled by the evidence-per-cell law (technique.md 3.10) so the bank's
#     operating point is held fixed while resolution grows:
#       half-life 250->1000, reserve 550->2200, reserve_residency 300->1200 (all ~ M),
#       graduation_hits 2->3 (~ 2 ln M argmin-stability; reachable because the 4x window drops
#       the rate-to-graduate). n_eff (effective hits) rises 721->2884.
#   - Oversample rises with the tilt AND is bumped for the finer bank: lo 4x, hi 8x (STARTING
#     values; tune up if thin_accept under-fills N=256, since finer M can shift the accept rate).
#   - GUARDRAIL: watch lam_spread at launch. Hold >= ~2.5 => co-scaling worked, finer M bought
#     resolution. Slide toward 1 => rare cells starving (dilution won), ABORT and lengthen the
#     half-life further (do NOT just raise M). Launch lo first, confirm ~20-30k steps, then hi.
#
...
exp_name="FMC_ViT-B_stab_${RUN}_rev12"
exp_dir="$BASE_DIR/$exp_name"

echo "========================================"
echo "Setup: $exp_name  (branch: $BRANCH)"
echo "  ViT-B/16 fixed-loader recipe (== rev3/rev5/rev6/rev7) + ONE ingredient: $RUN"
echo "  (rev12 = rev11 + bank M=32768 on thinned arms; H/reserve/residency/grad co-scaled; oversample lo 4x / hi 8x)"
echo "========================================"
...
echo "  Verifying rev12 hand-rolled DP + loss tuning + analytic w_max + thinned tilt fix is present..."
...
echo "    rev12 hand-rolled DP + loss tuning + analytic w_max + thinned tilt fix verified."
...
case "$RUN" in
  ...
  bc_thinned_lo)
    ...
    ensure_arg typicality_c_frac            0.25
    ensure_arg typicality_radius_mult       1.5
    # ---- REV12 bank M-scaling: M is the other FREE knob; the five below are DERIVED (technique 3.10) ----
    ensure_arg typicality_bank_size         32768               # 4x M -> finer catchments (morphology, not organ-type)
    ensure_arg typicality_halflife_steps    1000                # DERIVED ~M: restores per-cell hit-evidence, n_eff ~721->2884
    ensure_arg typicality_reserve_size      2200                # DERIVED ~M: absorbs 4x newborn inflow
    ensure_arg typicality_reserve_residency 1200                # DERIVED ~M: longer window to reach graduation
    ensure_arg typicality_graduation_hits   3                   # DERIVED ~ln M (argmin-stability 2 ln M); reachable via the 4x window
    ensure_arg thin_oversample_factor       4.0                 # DERIVED ~a, bumped for finer M. START; RAISE if thin_accept under-fills N=256.
  ...
  bc_thinned_hi)
    ...
    ensure_arg typicality_c_frac            0.25
    ensure_arg typicality_radius_mult       1.5
    # ---- REV12 bank M-scaling: identical DERIVED co-scale to bc_thinned_lo; only a and oversample differ ----
    ensure_arg typicality_bank_size         32768
    ensure_arg typicality_halflife_steps    1000
    ensure_arg typicality_reserve_size      2200
    ensure_arg typicality_reserve_residency 1200
    ensure_arg typicality_graduation_hits   3
    ensure_arg thin_oversample_factor       8.0                 # a=1.0 halves accept AND finer M may shift it -> 8x pool. START; RAISE if admits < N=256.
