#!/bin/bash
# run_sub_stab_suite_rev10.sh -- parametrized launcher for the "new-baseline" ViT-B ablations.
#
...
echo "  Verifying rev10 hand-rolled DP + loss tuning + analytic w_max is present..."
...
echo "    rev10 hand-rolled DP + loss tuning + analytic w_max verified."
...
case "$RUN" in
  ...
  bc_thinned_hi)
    ...
    ensure_arg typicality_c_frac            0.25
    ensure_arg typicality_radius_mult       1.5
    ensure_arg thin_oversample_factor       3.0                 # REV10: analytic w_max -> accept ~0.49. NB: acceptance tilt is hardcoded a=0.5 in trainer.py (NOT typicality_a), so hi accepts like lo -> 3.0 suffices. If that hardcode is ever fixed to a=1.0, RAISE this (harder tilt -> lower accept).
